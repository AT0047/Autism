...
document.getElementById('show-more-btn').addEventListener('click', function() {
    // يقوم الزر بعكس حالة العرض للمحتوى الإضافي
    var additionalContent = document.getElementById('additional-content');
    if (additionalContent.classList.contains('hidden')) {
        additionalContent.classList.remove('hidden');
        document.getElementById('show-more-btn').textContent = 'Hide';
    } else {
        additionalContent.classList.add('hidden');
        document.getElementById('show-more-btn').textContent = 'Show More';
    }
});
...
function toggleContent() {
    const currentDiv = contentDivs[currentIndex % contentDivs.length];
    const nextIndex = (currentIndex + 1) % contentDivs.length;
    const nextDiv = contentDivs[nextIndex];
...
// تحديث المحتوى كل ثانية (1000 مللي ثانية)
setInterval(toggleContent, 3000);

// تحديث المحتوى عند التحميل لأول مرة
toggleContent();
